...
class SignLanguageRecognizer:
    ...
    def __init__(self, model_path='results_v13_LSTM/sign_model_batch64.tflite'):
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            landmarks_frame = np.zeros(63)  # Initialize with zeros for one hand

            hand_count = 0
            if results.multi_hand_landmarks:
                for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
                    # Custom drawing for hand landmarks and connections
                    self.mp_drawing.draw_landmarks(
                        frame, hand_landmarks, self.mp_hands.HAND_CONNECTIONS,
                        self.mp_drawing.DrawingSpec(color=(121, 22, 76), thickness=3, circle_radius=4),
                        self.mp_drawing.DrawingSpec(color=(250, 44, 250), thickness=3, circle_radius=2),
                    )
